-- EMERGENCY RESET
-- This script uses a PL/pgSQL block to ignore errors if tables don't exist.

DO $$ 
BEGIN
    -- Attempt to drop tables. If they don't exist, it will skip.
    BEGIN
        EXECUTE 'DROP TABLE IF EXISTS public.okrs CASCADE';
    EXCEPTION WHEN OTHERS THEN NULL;
    END;

    BEGIN
        EXECUTE 'DROP TABLE IF EXISTS public.organization_members CASCADE';
    EXCEPTION WHEN OTHERS THEN NULL;
    END;
...
-- NOW RECREATE (Put this in the same script so you only run ONCE)
